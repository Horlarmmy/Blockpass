...
const MyTickets = () => {
  ...
  const { wallet } = useContext(NearContext); // Ensure this context is client-side only
  ...
  useEffect(() => {
    const loadUserTickets = async () => {
      if (!wallet) return;
      try {
        const tickets = []; // Replace with actual ticket fetching logic

        const combinedEvents = tickets.map((ticket, index) => ({
          id: BigNumber.isBigNumber(ticket.eventId)
            ? ticket.eventId.toString()
            : ticket.eventId,
          title: ticket.details[0] || "Untitled Event",
          date: ticket.details[1] || "N/A",
          startTime: ticket.details[2] || "N/A",
          endTime: ticket.details[3] || "N/A",
          location: ticket.details[4] || "Location not provided",
          imageUrl: localEvents[index]?.imageUrl || "/default-image.png",
    ...
    <>
      <nav className="container flex justify-between mx-auto items-center px-8 py-4">
        <Link href="/" passHref>
          ...
          >
            <Image src={logo} alt="BlockPass Logo" width={32} height={32} />
