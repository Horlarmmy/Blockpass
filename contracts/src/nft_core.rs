...
#[near_bindgen]
impl NonFungibleTokenCore for Blockpass {
    ...
    fn nft_transfer_call(
        ...
        ext_non_fungible_token_receiver
            ::ext(receiver_id.clone())
            .with_static_gas(GAS_FOR_NFT_ON_TRANSFER)
            .nft_on_transfer(sender_id, previous_token.owner_id.clone(), token_id.clone(), msg)
    ...
    fn nft_token(&self, token_id: TokenId) -> Option<JsonToken> {
        let token_id_u64 = U64::from(token_id.parse::<u64>().expect("Invalid token ID"));

        //if there is some token ID in the tokens_by_id collection
        if let Some(token) = self.tokens_by_id.get(&token_id_u64) {
...
#[near_bindgen]
impl NonFungibleTokenResolver for Blockpass {
    ...
    fn nft_resolve_transfer(
        ...
        let mut token = if let Some(token) = self.tokens_by_id.get(&token_id_u64) {
        ...
        self.tokens_by_id.insert(&token_id_u64, &token);
