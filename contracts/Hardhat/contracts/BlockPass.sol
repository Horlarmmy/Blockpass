 ...
 * @dev The contract implements block pass functionality with NFTs.
 */
contract BlockPass is ERC721URIStorage, Ownable {
    ...
    constructor(address initial_owner) Ownable(initial_owner) ERC721("BlockPass", "BP") {
        tokenId.increment();
    }
    ...
    function purchasePass(uint256 _blockPassId) public payable {
        ...
        (bool success, ) = payable(_pass.organizer).call{value: amountToOrganizer}("");
        require(success);
     ...
     */
    function allBlockPassList() public view returns (BlockPassDetails[] memory) {
